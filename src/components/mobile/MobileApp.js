  ...
  return (
    <div className={`m-app ${isDark ? 'm-dark' : 'm-light'}`}>
      ...
      <main className="m-body">
        {activeTab === TABS.HOME && (
          <HomeTab allGearboxes={allGearboxes} seriesStats={seriesStats} user={user} appData={appData} onNavigate={setActiveTab} />
        )}
        {activeTab === TABS.SELECTION && (
          <SelectionTab appData={appData} allGearboxes={allGearboxes} />
        )}
        {activeTab === TABS.PRODUCTS && (
          <ProductsTab allGearboxes={allGearboxes} />
        )}
        {activeTab === TABS.DOCS && (
          <DocsTab onSwitchToDesktop={onSwitchToDesktop} />
        )}
    ...
    <div className="m-page">
      ...
      <div className="m-chips">
        {seriesStats.map(([name, count]) => (
          <span key={name} className="m-chip">{name} <strong>{count}</strong></span>
        ))}
      </div>
...
// ==================== 产品库 Tab ====================
function ProductsTab({ allGearboxes }) {
...
// ==================== 文档 Tab ====================
function DocsTab({ onSwitchToDesktop }) {
  const sections = [
    { title: '文档中心', Icon: FileText, items: [
      { Icon: FileText, label: '技术询单', desc: '完整技术询价单生成' },
      { Icon: BarChart3, label: '报价单', desc: '产品报价管理' },
      { Icon: FileText, label: '技术协议', desc: '技术协议文档生成' },
      { Icon: FileText, label: '销售合同', desc: '销售合同管理' },
    ]},
    { title: '资料库', Icon: BookOpen, items: [
      { Icon: Image, label: '外形图库', desc: '产品外形图PDF' },
      { Icon: BookOpen, label: '说明书库', desc: '产品使用说明书' },
      { Icon: FileText, label: '协议模板库', desc: '历史技术协议模板' },
      { Icon: Settings, label: '配机案例', desc: '发动机匹配参考' },
    ]},
    { title: '推进系统', Icon: Compass, items: [
      { Icon: RotateCcw, label: '可调桨(CPP)', desc: 'CPP推进系统选型' },
      { Icon: Compass, label: '全回转推进器', desc: '全回转选型计算' },
      { Icon: Anchor, label: '侧推器', desc: '侧推器规格选型' },
      { Icon: Wrench, label: '轴系设计', desc: '传动轴系计算' },
    ]},
    { title: '分析工具', Icon: BarChart3, items: [
      { Icon: Zap, label: '扭振分析', desc: '扭振计算与分析' },
      { Icon: Zap, label: '能效分析', desc: '能效对比分析' },
      { Icon: BarChart3, label: '数据统计', desc: '选型数据统计' },
      { Icon: BarChart3, label: '竞品对比', desc: '16品牌竞品分析' },
    ]},
  ];
  ...
  return (
    <div className="m-page">
      <div className="m-notice">
        <Info size={16} />
        <span>以下功能在桌面版提供完整体验，点击下方按钮切换</span>
      </div>
        ...
        <div key={si} className="m-doc-section">
          ...
          <div className="m-doc-list">
            {sec.items.map((item, ii) => (
              <div key={ii} className="m-doc-item">
      ...
      <div className="m-switch-prompt">
        <p>需要完整功能？</p>
        <button className="m-btn m-btn-primary" onClick={onSwitchToDesktop}>
