...
const QuotationView = ({ 
  ...
  const hasPumpItem = quotation?.items?.some(item => item.name === "备用泵");
  const includePumpInGearbox = quotation?.options?.includePumpInGearbox;
  
  // 检查备用泵需求
  const checkPumpRequirement = () => {
    // 如果gearbox未定义，返回null
    if (!gearbox || !quotation) {
      return null;
    }
    
    const gearboxModel = gearbox.model || '未知型号';
    
    // 如果需要泵但报价单中没有
    if (requiresPump && !hasPumpItem && !includePumpInGearbox) {
      return {
        variant: "warning",
        message: `注意: 齿轮箱 ${gearboxModel} 需要配备备用泵，但报价单中未包含备用泵项目。`
      };
    }
    
    // 如果不需要泵但报价单中有
    if (!requiresPump && hasPumpItem) {
      return {
        variant: "info",
        message: `提示: 齿轮箱 ${gearboxModel} 不需要配备备用泵，但报价单中包含了备用泵项目。`
      };
    }
    ...
    if (requiresPump) {
      return {
        variant: "success",
        message: `齿轮箱 ${gearboxModel} 需要配备备用泵，且已包含在报价单中。`
      };
    } else {
      return {
        variant: "info",
  ...
  // 获取备用泵需求检查
  const pumpRequirementCheck = checkPumpRequirement();
  ...
  return (
    <Card className="mb-4 shadow-sm" style={{ backgroundColor: colors?.card || 'white', borderColor: colors?.border || '#ddd' }}>
      ...
      <Card.Body className="p-0">
        ...
        {/* 备用泵需求检查提示 */}
        {pumpRequirementCheck && (
          <Alert variant={pumpRequirementCheck.variant} className="m-3">
            <i className="bi bi-info-circle-fill me-2"></i>
            <strong>备用泵需求检查：</strong> {pumpRequirementCheck.message}
          </Alert>
        )}
