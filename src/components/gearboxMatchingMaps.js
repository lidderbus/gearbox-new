 ...
 * value: 推荐的联轴器型号前缀
 */
const gearboxToCouplingPrefixMap = {
  // 300系列
  'HC300': 'HGTHT4',
  '300': 'HGTHT4',
  'D300A': 'HGTHT4',
 ...
 */
export const couplingSpecificationsMap = { // ADDED export keyword here
  'HGTHT4': {
  ...
  // Ensure all models in flexibleCouplings have an entry here if possible
};
...
export const getRecommendedCouplingInfo = (gearboxModel, hasCover = false) => {
  ...
  // 检查是否需要根据罩壳状态调整型号
  if (hasCover && specificMatch && couplingWithCoverMap[specificMatch]) {
     specificMatch = couplingWithCoverMap[specificMatch];
  } else if (hasCover && !specificMatch) {
      // If no specific match but cover is needed, try adding typical covered suffixes to prefix matches
      // This is a fallback and might not be accurate for all prefixes
      ...
      for (const prefix in gearboxToCouplingPrefixMap) {
       ...
       if (prefixMatch && prefixMatch.startsWith('HGTHB')) {
           specificMatch = `${prefixMatch.replace('HGTHB', 'HGTHJB')}`; // Add JB suffix for covered HGTHB
            console.warn(`Fallback: Gearbox ${gearboxModel} needed cover, no specific match, using estimated covered prefix match: ${specificMatch}`);
       } else if (prefixMatch && prefixMatch.startsWith('HGT')) {
            specificMatch = `${prefixMatch}-ZB`; // Add -ZB suffix for covered HGT
            console.warn(`Fallback: Gearbox ${gearboxModel} needed cover, no specific match, using estimated covered prefix match: ${specificMatch}`);
       } else {
            // No useful prefix or unable to guess covered version, use default and warn
            prefixMatch = gearboxToCouplingPrefixMap.default;
            console.warn(`Fallback: Gearbox ${gearboxModel} needed cover, no specific/prefix match, using default prefix: ${prefixMatch}`);
            return {
               prefix: prefixMatch,
               specific: null // No specific model guessed
            };
       }
  }
  ...
  const prefixMatch = specificMatch.match(/^([A-Z]+)/)?.[0]; // Use optional chaining
  if (!prefixMatch) {
      console.warn(`Could not extract prefix from recommended coupling model ${specificMatch}`);
      // Fallback to a generic prefix
      return {
         prefix: 'HGT', // Or some other default/error indicator
