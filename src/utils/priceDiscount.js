...
export const calculateFactoryPrice = (item) => {
  ...
  // 确保折扣率使用小数表示 (0-1范围) 而不是百分比 (0-100范围)
  let discountRate = typeof item.discountRate === 'number' ? item.discountRate : getDiscountRate(item.model);
  
  // 修正可能的折扣率格式错误（如果大于1，则视为百分比格式）
  if (discountRate > 1) {
    console.warn(`检测到可能的折扣率格式错误：${discountRate}，自动转换为小数`);
...
export const calculateMarketPrice = (item, packagePrice) => {
  ...
  // 如果都没有，则基于基础价格和折扣率计算
    ...
    const discountRate = typeof item.discountRate === 'number' ? 
                          (item.discountRate > 1 ? item.discountRate / 100 : item.discountRate) : 
                          getDiscountRate(item.model);
