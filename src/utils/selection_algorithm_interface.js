...
// 导入算法
import { traditionalSelectionAlgorithm } from './traditionalSelection';
import { GeneticSelectionEngine } from './geneticSelectionEngine';
import { calculateScore } from '../utils/scoreUtils';
import { sortGearboxResults } from '../utils/sortUtils';
...
export function initializeGeneticEngine(options = {}) {
  try {
    geneticEngine = new GeneticSelectionEngine(options);
...
async function runGeneticSelection(gearboxData, requirements, options) {
  ...
  if (!geneticEngine) {
    geneticEngine = new GeneticSelectionEngine({
      // 使用自定义评分权重（如果提供）
...
function runTraditionalSelection(gearboxData, requirements, options) {
  // 使用传统算法进行选型
  const traditionalResults = traditionalSelectionAlgorithm(gearboxData, requirements);
  
  // 使用遗传算法的评分机制重新评分
  const scoredResults = traditionalResults.map(gearbox => {
    // 创建临时遗传算法引擎实例计算评分
    const tempEngine = new GeneticSelectionEngine(options.scoringWeights || {});
  ...
  // 按适应度排序
  return scoredResults.sort((a, b) => b.fitness - a.fitness);
}
