...
} from './classificationCertificates';
...
export const generateAgreement = (selectionResult, projectInfo, selectedComponents) => {
  ...
  try {
    ...
    // 获取船检类型（从projectInfo或使用默认值）
    const classificationType = projectInfo?.classificationType || ClassificationType.NONE;
    const classificationName = getClassificationDisplayName(classificationType);
    const certRequirements = getCertificateRequirements(classificationType);

    // 动态生成技术文件清单（根据船检类型）
    const quantity = projectInfo?.quantity || 1;
    const standardDocuments = generateCertificateList(classificationType, quantity);

    // 添加基础技术文件（如果船检配置中没有）
    const baseDocuments = ["总装图 2份", "电气控制原理图 2份"];
    baseDocuments.forEach(doc => {
      if (!standardDocuments.some(d => d.includes(doc.split(' ')[0]))) {
    ...
    const agreement = {
      ...
      content: {
        ...
        classificationInfo: {
          ...
          society: certRequirements.society,
          requiresInspection: requiresOnSiteInspection(classificationType),
          inspectionRequirements: certRequirements.inspectionRequirements || [],
          specialNotes: certRequirements.specialNotes || []
        },
        qualityAssurance: {
          testStandard: classificationType !== ClassificationType.NONE
            ? `产品出厂前按${classificationName}规范测试`
            : "产品出厂前按国家标准测试",
          warranty: "产品自交付之日起12个月内质保",
          inspection: requiresOnSiteInspection(classificationType)
            ? `需经${certRequirements.society || '船检'}验船师现场检验`
            : null
        }
      }
    };
 ...
 */
export const exportAgreementToWord = async (agreement, filename = '船用齿轮箱技术协议', selectedDrawings = []) => {
  ...
  // 动态加载 docx
  const { Document, Packer, Paragraph, Table, TableRow, TableCell, HeadingLevel, TextRun, WidthType, BorderStyle } = await loadDocx();
  ...
  try {
    const children = [
      ...
      // 备用泵 (如果存在)
       ...
       ...(() => { // 使用 IIFE 来包含逻辑并返回数组
            ...
            if (technicalDocuments.length > 0) {
             ...
             let qaIndex = 1;
             sections.push(new Paragraph({ text: `${qaIndex++}. ${qualityAssurance.testStandard || "产品出厂前按国家标准测试"}`, indentation: { left: 720 } }));
             sections.push(new Paragraph({ text: `${qaIndex++}. ${qualityAssurance.warranty || "产品自安装调试完成后18个月内质保"}`, indentation: { left: 720 } }));
             // 添加船检验收要求（如果有）
             if (qualityAssurance.inspection) {
               sections.push(new Paragraph({ text: `${qaIndex++}. ${qualityAssurance.inspection}`, indentation: { left: 720 } }));
             }
             sections.push(new Paragraph({ text: `${qaIndex++}. 未尽事宜友好协商解决。`, indentation: { left: 720 } }));
      ...
      ...(selectedDrawings.length > 0 ? [
        ...
        createSectionTitle('附件清单 — 外形图'),
        new Table({
          width: { size: 9000, type: WidthType.DXA },
          rows: [
            new TableRow({
              children: [
                new TableCell({ width: { size: 600, type: WidthType.DXA }, children: [new Paragraph({ text: '序号', alignment: 'center' })] }),
                new TableCell({ width: { size: 2000, type: WidthType.DXA }, children: [new Paragraph('型号')] }),
                new TableCell({ width: { size: 4000, type: WidthType.DXA }, children: [new Paragraph('文件名')] }),
                new TableCell({ width: { size: 1200, type: WidthType.DXA }, children: [new Paragraph({ text: '类型', alignment: 'center' })] }),
                new TableCell({ width: { size: 1200, type: WidthType.DXA }, children: [new Paragraph({ text: '文件大小', alignment: 'center' })] }),
              ],
            }),
            ...selectedDrawings.map((drawing, idx) => new TableRow({
              children: [
                new TableCell({ width: { size: 600, type: WidthType.DXA }, children: [new Paragraph({ text: `${idx + 1}`, alignment: 'center' })] }),
                new TableCell({ width: { size: 2000, type: WidthType.DXA }, children: [new Paragraph(drawing.model || '-')] }),
                new TableCell({ width: { size: 4000, type: WidthType.DXA }, children: [new Paragraph(drawing.fileName || '-')] }),
                new TableCell({ width: { size: 1200, type: WidthType.DXA }, children: [new Paragraph({ text: drawing.type === 'gearbox' ? '齿轮箱' : drawing.type === 'coupling' ? '联轴器' : '-', alignment: 'center' })] }),
                new TableCell({ width: { size: 1200, type: WidthType.DXA }, children: [new Paragraph({ text: drawing.fileSize || '-', alignment: 'center' })] }),
              ],
            })),
          ],
        }),
        new Paragraph({ text: `共 ${selectedDrawings.length} 份外形图文件`, spacing: { before: 100 }, style: { font: { size: 18, color: '666666' } } }),
      ] : []),
