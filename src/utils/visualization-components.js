...
import { Card, CardContent, Button, Grid, Typography, Box, Divider, TextField, MenuItem, Slider, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper } from '@mui/material';
        ...
        {radarData.map((entry, index) => {
          ...
          return (
            <Radar
              ...
              fillOpacity={0.3}
              onClick={() => onClick && onClick(entry._index)}
...
export const GearboxComparisonChart = ({ gearboxes, compareField = 'score', height = 350, onClick }) => {
  ...
  const chartData = useMemo(() => {
    return gearboxes.map(gearbox => {
      // 对于评分字段，从fitnessDetails或scoreDetails中获取
      const details = gearbox.fitnessDetails || gearbox.scoreDetails || {};
      let fieldValue = 0;
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <BarChart data={chartData}>
          ...
          <YAxis />
          <Tooltip
            formatter={(value) => [value.toFixed(2), comparableFields.find(f => f.key === selectedField)?.name]}
          />
          <Legend />
          <Bar
            dataKey="value"
