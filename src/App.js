...
// 导入新增的功能模块
import { validateGearboxData, cleanGearboxData, repairGearboxData, 
         batchValidateGearboxData, batchCleanGearboxData, 
         detectAndFixOutliers, processDataImport } from './utils/data-validation-module';
import { GeneticSelectionEngine } from './utils/genetic-selection-algorithm';
import { initializeGeneticEngine, selectGearbox as geneticSelectGearbox, evaluateGearbox, 
         compareGearboxes } from './utils/selection-algorithm-interface';
import { PerformanceRadarChart, GearboxComparisonChart, EfficiencyCurveChart,
         ScoreChart, ScoreBreakdownChart, GearboxComparisonDashboard,
         ParametersInputForm, SelectionResultView } from './components/visualization-components';
import { formatCurrency, convertCurrency, fetchExchangeRates,
         generatePaymentSchedule, generateIncotermQuotations,
         generateMultiCurrencyQuotation, MultiCurrencyQuotationTable,
         IncotermSelector, PaymentSchedule } from './utils/quote-system-enhancement';
...
function useGearboxSelection({
  ...
  calculateAllPrices,
  correctPriceData,
  getGWPackagePriceConfig,
  enhancedCouplingSelection,
  enhancedSelectPump,
  needsStandbyPump,
  ...
  const handleGearboxSelection = useCallback((index) => {
    ...
    if (!selectionResult) {
      ...
      return { success: false, message: "无法选择齿轮箱，请先进行选型。" };
    }
    ...
    if (!Array.isArray(selectionResult.recommendations)) {
      ...
      return { success: false, message: "无法选择齿轮箱，选型结果格式错误。" };
    }
    ...
    if (index < 0 || index >= selectionResult.recommendations.length) {
      ...
      return { success: false, message: `无法选择齿轮箱，索引 ${index} 超出范围。` };
    }
    ...
    try {
      ...
      if (!appDataState || !Array.isArray(appDataState.flexibleCouplings) || !Array.isArray(appDataState.standbyPumps)) {
        ...
        setActiveTab('result');
        return { success: true, message: "齿轮箱选择成功，但配件数据不完整。" };
      }
      ...
      setSelectedComponents(prev => ({
        ...prev,
        gearbox: correctedGearbox,
        coupling: updatedCouplingResult?.model ? correctPriceData(updatedCouplingResult) : null,
        pump: updatedPumpResult?.model ? correctPriceData(updatedPumpResult) : null
      }));

      // 使用统一的价格计算函数更新价格
      const newPriceData = calculateAllPrices(
        correctedGearbox,
        updatedCouplingResult?.model ? correctPriceData(updatedCouplingResult) : null,
        updatedPumpResult?.model ? correctPriceData(updatedPumpResult) : null,
      ...
      return { 
        ...
        message: "齿轮箱选择成功",
        gearbox: correctedGearbox,
        coupling: updatedCouplingResult,
        pump: updatedPumpResult
      };
    } catch (error) {
      console.error("选择齿轮箱时发生错误:", error);
      setError("选择齿轮箱过程中发生错误: " + (error.message || "未知错误"));
      return { 
        success: false, 
        message: "选择齿轮箱过程中发生错误: " + (error.message || "未知错误")
      };
    }
  }, [
    selectionResult,
  ...
  // 处理选型
  const handleSelectGearbox = useCallback(async () => {
    const validation = isFormValid();
    if (!validation.isValid) {
      setError('请先填写完整的选型参数');
      return;
    }

    // 检查应用数据
    if (!appDataState || Object.keys(appDataState).length === 0) {
      setError('应用数据未加载完成，请刷新页面重试');
      return;
    }
    ...
    const missingCollections = requiredCollections.filter(collection => 
      !Array.isArray(appDataState[collection]) || appDataState[collection].length === 0
    );

    if (missingCollections.length > 0) {
      setError(`数据不完整，缺少: ${missingCollections.join(', ')}。请刷新页面重试。`);
      return;
    }

    // 如果选择的是auto但数据不足，自动改为HC或其他可用类型
    let effectiveGearboxType = gearboxType;
    if (gearboxType === 'auto') {
      const availableTypes = ['HC', 'GW', 'HCM', 'DT', 'HCQ', 'GC'].filter(type => 
        Array.isArray(appDataState[`${type.toLowerCase()}Gearboxes`]) && 
        appDataState[`${type.toLowerCase()}Gearboxes`].length > 0
      );
      
      if (availableTypes.length === 0) {
        setError('没有可用的齿轮箱数据，无法进行选型');
        return;
      }
      
      effectiveGearboxType = availableTypes[0];
      console.log(`自动转换类型: 从"auto"改为"${effectiveGearboxType}"`);
    }
    ...
    try {
      console.log("选型参数:", {
        motorPower: engineData.power,
        motorSpeed: engineData.speed,
        targetRatio: requirementData.targetRatio,
        thrust: requirementData.thrustRequirement,
        workCondition: requirementData.workCondition,
        temperature: requirementData.temperature,
        hasCover: requirementData.hasCover,
        application: requirementData.application,
        effectiveType: effectiveGearboxType
      });

      const result = await selectGearbox(
        parseFloat(engineData.power),
        parseFloat(engineData.speed),
        parseFloat(requirementData.targetRatio),
        parseFloat(requirementData.thrustRequirement) || 0,
        effectiveGearboxType,
        appDataState,
        {
          workCondition: requirementData.workCondition,
          temperature: requirementData.temperature,
      ...
      if (result.success && result.recommendations && result.recommendations.length > 0) {
        if (gearboxType === 'auto' && effectiveGearboxType !== 'auto') {
          setSuccess(`选型成功！(已自动使用${effectiveGearboxType}系列)`);
        } else {
          setSuccess('选型成功！');
        }
        setSelectionResult(result);
        setActiveTab('result');
        await handleGearboxSelection(0);
      } else {
        setError(result.message || '未找到合适的齿轮箱');
      }
    } catch (error) {
      console.error('选型过程中发生错误:', error);
      setError('选型过程中发生错误: ' + (error.message || '未知错误'));
    } finally {
      setLoading(false);
    }
  }, [appDataState, engineData, requirementData, gearboxType, isFormValid, handleGearboxSelection, 
      setLoading, setError, setSuccess, setActiveTab, setSelectionResult]);
  ...
  }, [selectionResult, selectedComponents, priceData.includePump, calculateAllPrices, correctPriceData, setSelectionResult, setSelectedComponents, setPriceData, setPackagePrice, setMarketPrice, setTotalMarketPrice, setQuotation, setAgreement, setContract, setSuccess, setError]);
...
function App({ appData: initialAppData, setAppData }) {
  ...
  // 添加新的遗传算法引擎状态
  const [geneticEngineInitialized, setGeneticEngineInitialized] = useState(false);
  // 添加多货币与贸易条款状态
  const [currentCurrency, setCurrentCurrency] = useState('CNY');
  const [exchangeRates, setExchangeRates] = useState(null);
  const [selectedIncoterm, setSelectedIncoterm] = useState('EXW');
  const [incotermQuotations, setIncotermQuotations] = useState({});
  const [paymentSchedule, setPaymentSchedule] = useState(null);
  // 添加数据验证状态
  const [dataValidationResults, setDataValidationResults] = useState(null);
  const [dataRepairResults, setDataRepairResults] = useState(null);
  ...
  useEffect(() => {
    ...
    const initEngine = async () => {
      try {
        const result = await initializeGeneticEngine({
          populationSize: 50,
          generations: 30,
          // 自定义权重
          weightEfficiency: 25,
          weightTorqueReserve: 20,
          weightTemperature: 15,
          weightNoise: 10,
          weightCost: 15,
          weightLifespan: 15
        });
        ...
        if (result.success) {
          setGeneticEngineInitialized(true);
          setSuccess('遗传算法引擎初始化成功');
        } else {
          setError(result.message || '遗传算法引擎初始化失败');
        }
      } catch (error) {
        console.error('初始化遗传算法引擎时出错:', error);
        setError('初始化遗传算法引擎失败: ' + error.message);
      }
    };
    ...
    const fetchRates = async () => {
      try {
        ...
        setError('获取汇率数据失败: ' + error.message);
      }
    };
  ...
  useEffect(() => {
    if (!appDataState || Object.keys(appDataState).length === 0) {
      console.warn("应用数据未完全加载，某些功能可能不可用");
      setError("数据加载不完整，功能可能受限。如果问题持续，请刷新页面。");
      return;
    }
    ...
    if (missingCollections.length > 0) {
      console.warn("缺少必要的数据集合:", missingCollections);
      setError(`数据不完整，缺少: ${missingCollections.join(', ')}。某些功能可能不可用。`);
    } else {
      console.log("应用数据加载完成，所有必要集合都存在");
      setDataChecked(true);
    }
  }, [appDataState]);

  // 在 selectionResult 更新时的处理
  useEffect(() => {
  ...
  }, [selectionResult, calculateAllPrices, correctPriceData, getGWPackagePriceConfig]);
  ...
  const handlePriceChange = useCallback((component, field, value) => {
    setSelectedComponents(prev => {
      ...
      if (updatedComponents[component]) {
        updatedComponents[component] = {
          ...updatedComponents[component],
          [field]: field === 'discountRate' ? value / 100 : value
        };
      }
    ...
    setMarketPrice(newPriceData.marketPrice);
    setTotalMarketPrice(newPriceData.totalMarketPrice);
  }, [selectedComponents, calculateAllPrices, setPriceData, setPackagePrice, setMarketPrice, setTotalMarketPrice]);
  ...
  // 添加数据验证处理函数
  const handleValidateData = useCallback(async (data) => {
    setLoading(true);
    
    try {
      // 执行数据验证
      const validationResult = Array.isArray(data) 
        ? batchValidateGearboxData(data)
        : validateGearboxData(data);
      
      // 更新验证结果
      setDataValidationResults(validationResult);
      
      if (validationResult.valid) {
        setSuccess('数据验证成功');
      } else {
        setError(`数据验证失败: ${validationResult.invalidCount || 1}个无效项目`);
      }
      
      return validationResult;
    } catch (error) {
      console.error('数据验证失败:', error);
      setError('数据验证失败: ' + error.message);
  ...
  // 添加数据修复处理函数
  const handleRepairData = useCallback(async (data) => {
    setLoading(true);
    
    try {
      // 执行数据修复
      const repairResults = Array.isArray(data)
        ? await Promise.all(data.map(item => repairGearboxData(item)))
        : repairGearboxData(data);
      
      // 更新修复结果
      setDataRepairResults(repairResults);
      
      if (Array.isArray(repairResults)) {
        const successCount = repairResults.filter(result => result.success).length;
        setSuccess(`数据修复完成: ${successCount}/${repairResults.length}个项目修复成功`);
      } else if (repairResults.success) {
        setSuccess(`数据修复成功: ${repairResults.repairs.length}处问题已修复`);
      } else {
        setError('数据修复失败');
      }
  ...
  const handleCurrencyChange = useCallback((newCurrency) => {
    ...
    try {
      // 转换报价单到新货币
      const convertedQuotation = generateMultiCurrencyQuotation(
        quotation,
        newCurrency,
        exchangeRates
      );
      ...
      setSuccess(`报价单已转换为${newCurrency}`);
    } catch (error) {
      console.error('货币转换失败:', error);
      setError('货币转换失败: ' + error.message);
    } finally {
      setLoading(false);
    }
  ...
  // 处理国际贸易条款变更函数
  const handleIncotermChange = useCallback((newIncoterm) => {
    if (!quotation) return;
    ...
    try {
      // 如果已经有此贸易条款的报价单，直接使用
      if (incotermQuotations[newIncoterm]) {
        setQuotation(incotermQuotations[newIncoterm]);
        setSelectedIncoterm(newIncoterm);
        setSuccess(`已切换到${newIncoterm}贸易条款`);
        setLoading(false);
        return;
      }
      ...
      const incotermOptions = {
        baseIncoterm: 'EXW',
        targetIncoterms: [newIncoterm],
        originCountry: 'CN',
        destinationCountry: 'US',
        shippingPort: '上海',
        destinationPort: '洛杉矶',
        transportCost: quotation.totalAmount * 0.05, // 示例：运费为5%
        insuranceCost: quotation.totalAmount * 0.02, // 示例：保险费为2%
        exportClearanceCost: 3000,                  // 示例：出口清关费
        importClearanceCost: 5000,                  // 示例：进口清关费
        importDuty: quotation.totalAmount * 0.08,   // 示例：进口关税为8%
        currency: quotation.currency
      };
      
      const newQuotations = generateIncotermQuotations(quotation, incotermOptions);
      
      // 更新报价单和状态
      setQuotation(newQuotations[newIncoterm]);
      setIncotermQuotations(prev => ({ ...prev, ...newQuotations }));
      setSelectedIncoterm(newIncoterm);
      setSuccess(`已生成${newIncoterm}贸易条款报价单`);
    } catch (error) {
      console.error('生成贸易条款报价单失败:', error);
      setError('生成贸易条款报价单失败: ' + error.message);
    } finally {
      setLoading(false);
    }
  }, [quotation, incotermQuotations]);

  // 处理付款计划生成
  const handleGeneratePaymentSchedule = useCallback(() => {
    if (!quotation) return;
    ...
    try {
      ...
      setError('生成付款计划失败: ' + error.message);
    }
  }, [quotation]);

  // 处理付款计划更新
  const handlePaymentScheduleUpdate = useCallback((updatedSchedule) => {
    if (!quotation) return;
    ...
    try {
      ...
      setError('更新付款计划失败: ' + error.message);
    }
  }, [quotation]);
  ...
  return (
    <Container fluid className="app-container" style={{ backgroundColor: colors.bg, color: colors.text }}>
      ...
      >
                ...
                <Col>
                  ...
                  {selectionResult && (
                    <Row>
                      <Col>
                        {/* 性能雷达图 */}
                        <Card className="mb-4 shadow-sm" style={{ backgroundColor: colors.card }}>
                          <Card.Header style={{ backgroundColor: colors.headerBg, color: colors.headerText }}>
                            <i className="bi bi-graph-up me-2"></i>性能六维评分
                          </Card.Header>
                          <Card.Body>
                            <PerformanceRadarChart
                              gearboxes={selectionResult.recommendations || selectionResult.results}
                              height={400}
                              onClick={handleGearboxSelection}
                            />
                          </Card.Body>
                        </Card>
                        ...
                        <Card className="mb-4 shadow-sm" style={{ backgroundColor: colors.card }}>
                          <Card.Header style={{ backgroundColor: colors.headerBg, color: colors.headerText }}>
                            <i className="bi bi-bar-chart-line me-2"></i>综合评分对比
                          </Card.Header>
                          <Card.Body>
                            <ScoreChart
                              gearboxes={selectionResult.recommendations || selectionResult.results}
                    ...
                    <Row>
                      ...
                      <Col md={6}>
                        <EfficiencyCurveChart
                          gearboxData={selectedComponents.gearbox}
                          title="效率曲线"
                        />
                      </Col>
                    </Row>
                            ...
                            <div className="mt-3">
                              {paymentSchedule ? (
                                <PaymentSchedule
                                  ...
                                  onScheduleChange={handlePaymentScheduleUpdate}
                                  editable={!loading}
                                  currency={quotation.currency}
                                />
                              ) : (
                                <Button
                                  onClick={handleGeneratePaymentSchedule}
                                  disabled={loading}
                                  variant="outline-primary"
                                  className="w-100"
                                >
          ...
          {/* 新增数据管理功能卡片 */}
          <Tab eventKey="data-management" title={<span><i className="bi bi-database-gear me-1"></i>数据管理</span>}>
            <Row>
              <Col>
                {/* 数据验证与修复功能卡片 */}
                <Card className="mb-4 shadow-sm" style={{ backgroundColor: colors.card }}>
                  <Card.Header style={{ backgroundColor: colors.headerBg, color: colors.headerText }}>
                    <i className="bi bi-shield-check me-2"></i>数据验证与修复
                  </Card.Header>
                  <Card.Body>
                    <Row>
                      <Col md={6}>
                        <Button
                          onClick={() => handleValidateData(appDataState.allGearboxes)}
                          disabled={loading || !appDataState.allGearboxes}
                          variant="outline-primary"
                          className="w-100 mb-2"
                        >
                      ...
                      <Col md={6}>
                        <Button
                          onClick={() => handleRepairData(appDataState.allGearboxes)}
                          disabled={loading || !appDataState.allGearboxes}
                          variant="outline-success"
                          className="w-100 mb-2"
                        >
                    ...
                    {dataValidationResults && (
                      <Alert variant={dataValidationResults.valid ? 'success' : 'warning'} className="mt-3">
                        <Alert.Heading>数据验证结果</Alert.Heading>
                        <p>
                          {`总项目数: ${dataValidationResults.totalCount || 1}，`}
                          {`有效项目: ${dataValidationResults.validCount || (dataValidationResults.valid ? 1 : 0)}，`}
                          {`无效项目: ${dataValidationResults.invalidCount || 0}`}
                        </p>
                        {!dataValidationResults.valid && dataValidationResults.allErrors && (
                          <div>
                            <strong>错误信息:</strong>
                            <ul>
                              {dataValidationResults.allErrors.slice(0, 5).map((error, index) => (
                                <li key={index}>{error}</li>
                              ))}
                              {dataValidationResults.allErrors.length > 5 && (
                                <li>... 还有 {dataValidationResults.allErrors.length - 5} 个错误</li>
                              )}
                            </ul>
                          </div>
                        )}
                    ...
                    {dataRepairResults && (
                      <Alert variant="info" className="mt-3">
                        <Alert.Heading>数据修复结果</Alert.Heading>
                        {Array.isArray(dataRepairResults) ? (
                          <p>
                            {`总项目数: ${dataRepairResults.length}，`}
                            {`修复成功: ${dataRepairResults.filter(r => r.success).length}，`}
                            {`修复失败: ${dataRepairResults.filter(r => !r.success).length}`}
                          </p>
                        ) : (
                          <>
                            <p>{dataRepairResults.success ? '修复成功' : '修复失败'}</p>
                            {dataRepairResults.success && dataRepairResults.repairs && (
                              <div>
                                <strong>修复操作:</strong>
                                <ul>
                                  {dataRepairResults.repairs.map((repair, index) => (
                ...
                <Card className="mb-4 shadow-sm" style={{ backgroundColor: colors.card }}>
                  <Card.Header style={{ backgroundColor: colors.headerBg, color: colors.headerText }}>
                    <div className="d-flex justify-content-between align-items-center">
                      <span><i className="bi bi-cash-stack me-2"></i>价格管理</span>
                      <Button 
                        variant="outline-primary" 
                        size="sm" 
                        onClick={() => setShowBatchPriceAdjustment(true)}
                      >
                        <i className="bi bi-pencil-square me-1"></i> 批量调整价格
                      </Button>
                    </div>
                  </Card.Header>
                  <Card.Body>
